...
for (i in colnames(filtered)[1:l]) {
  ...
  if (d[5] != ".") {
    condition <- cbind(condition, "mutant")
  } else {
    condition <- cbind(condition, str_replace(d[4], " ", ""))
  }
}
...
for (i in colnames(a)[1:155]) {
  ...
  d <- sample.annotation[sample.annotation["Sample.ID"]==x[1]]
  if (d[5] != ".") {
    rename.cols <- cbind(rename.cols,  paste("mutant", x[1], sep = "."))
  } else {
    rename.cols <- cbind(rename.cols, paste(str_replace(d[4], " ", ""), x[1], sep = "."))
  }
}
...
write.table(fin.df, "H:/20230713_143813_22_phospho_no-norm_Report.output.de.0.3mutant-iPD.tsv", sep="\t", row.names = FALSE)
...
write.table(raw.df, "H:/20230713_143813_22_phospho_no-norm_Report.output.raw.mutant.tsv", sep="\t", row.names = FALSE)