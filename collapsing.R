...
inFile <- "E:/20230713_143813_22_phospho_no-norm_Report.tsv"
outFile <- "E:/20230713_143813_22_phospho_no-norm_Report.output.tsv"

ptm.string <- "[Phospho (STY)];[Deamidation (NQ)];[Oxidation (M)];[Carbamidomethyl (C)]"
main.col <- "EG.TotalQuantity..Settings."

csv.file <- read.table(inFile, header=TRUE, sep="\t", fill = TRUE )
...
#Perseus Data Read
mdata <- csv.file
paramFile <- xmlParse("C:/Users/Toan Phung/Downloads/parameters.xml")
...
findf <- cbind(mdata.main, mdata.annotCols)
write.table(findf, outFile, sep="\t", row.names = FALSE)
