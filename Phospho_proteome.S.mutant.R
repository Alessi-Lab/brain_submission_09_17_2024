...
for (i in colnames(filtered)[1:l]) {
  ...
  if (d[7] == "S") {
    if (d[5] != ".") {
      condition <- cbind(condition,  str_replace(d[5], " ",""))
    } else {
...
contrast.matrix <- makeContrasts(G2019S-Control, levels=design)
fit2 <- contrasts.fit(fit, contrast.matrix)
fit2 <- eBayes(fit2)
result <- topTable(fit2, coef=1, number=Inf, sort.by = "none")
fin.df <- cbind(rowData(d[[4]]), result)
write.table(fin.df, "H:/20230713_143813_22_phospho_no-norm_Report.output.de.0.3.S.G2019S-Control.tsv", sep="\t", row.names = FALSE)
